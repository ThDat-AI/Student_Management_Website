...
import { Container, Row, Col, Button, Card, Spinner, Form, Modal, Badge, InputGroup } from "react-bootstrap";
...
const QuyDinhModal = ({ show, onHide, mode, selectedQuyDinh, latestQuyDinh, onSubmit }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!validateForm()) {
      toast.warn("Vui lòng kiểm tra lại các thông tin đã nhập");
      return;
    }
    ...
    <Modal show={show} onHide={onHide} size="lg" backdrop="static">
      <Form onSubmit={handleFormSubmit}>
        <Modal.Header closeButton>
          <Modal.Title>{isEditMode ? `Chỉnh sửa quy định: ${selectedQuyDinh?.TenNienKhoa}` : "Thêm niên khóa và quy định mới"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {/* Alert đã được xóa khỏi đây */}
          ...
          <Row>
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                <Form.Control type="number" name="TuoiToiThieu" value={formData.TuoiToiThieu || ""} onChange={handleInputChange} isInvalid={!!validationErrors.TuoiToiThieu} required />
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                <Form.Control type="number" name="TuoiToiDa" value={formData.TuoiToiDa || ""} onChange={handleInputChange} isInvalid={!!validationErrors.TuoiToiDa} required />
          ...
          <Row>
            <Col md={4}>
              <Form.Group className="mb-3">
                ...
                <Form.Control type="number" name="SiSoToiDa" value={formData.SiSoToiDa || ""} onChange={handleInputChange} isInvalid={!!validationErrors.SiSoToiDa} required />
            ...
            <Col md={4}>
              <Form.Group className="mb-3">
                ...
                <Form.Control type="number" name="SoMonHocToiDa" value={formData.SoMonHocToiDa || ""} onChange={handleInputChange} required />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Label>Điểm đạt môn</Form.Label>
                <Form.Control type="number" step="0.1" name="DiemDatMon" value={formData.DiemDatMon || ""} onChange={handleInputChange} isInvalid={!!validationErrors.DiemDatMon} required />
          ...
          <Row>
            <Col md={4}>
              <Form.Group className="mb-3">
                ...
                <Form.Control type="number" name="SoLopK10" value={formData.SoLopK10 || ""} onChange={handleInputChange} required />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Label>Số lớp khối 11</Form.Label>
                <Form.Control type="number" name="SoLopK11" value={formData.SoLopK11 || ""} onChange={handleInputChange} required />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group className="mb-3">
                <Form.Label>Số lớp khối 12</Form.Label>
                <Form.Control type="number" name="SoLopK12" value={formData.SoLopK12 || ""} onChange={handleInputChange} required />
              </Form.Group>
            </Col>
          </Row>
        </Modal.Body>
    ...
    <div className="table-responsive">
      <table className="table table-hover align-middle">
        <thead className="table-light">
          <tr>
            <th className="text-nowrap"><FaGraduationCap className="me-2 text-primary" />Niên khóa</th>
            <th className="text-center text-nowrap"><FaUsers className="me-2 text-info" />Độ tuổi</th>
            <th className="text-center text-nowrap"><FaUsers className="me-2 text-success" />Sĩ số tối đa</th>
            <th className="text-center text-nowrap"><FaClipboardList className="me-2 text-warning" />Số môn tối đa</th>
            <th className="text-center text-nowrap"><FaGraduationCap className="me-2 text-danger" />Điểm đạt</th>
            ...
            <tr key={qd.IDNienKhoa} className="border-bottom">
              <td>
                <div className="d-flex align-items-center">
                  <div className="bg-primary rounded-circle p-2 me-3"><FaGraduationCap className="text-white" size={14} /></div>
                  <div>
                    <strong className="text-dark">{qd.TenNienKhoa}</strong><br />
                    <small className="text-muted">Niên khóa học</small>
                  </div>
                </div>
              </td>
              <td className="text-center"><Badge bg="info" className="px-3 py-2">{qd.TuoiToiThieu} - {qd.TuoiToiDa} tuổi</Badge></td>
              <td className="text-center"><div className="fw-bold text-success fs-5">{qd.SiSoToiDa}</div><small className="text-muted">học sinh</small></td>
              <td className="text-center"><div className="fw-bold text-warning fs-5">{qd.SoMonHocToiDa}</div><small className="text-muted">môn học</small></td>
              <td className="text-center"><Badge bg="primary" className="fs-6 px-3 py-2">{qd.DiemDatMon}</Badge></td>
              <td className="text-center">
                <div className="d-flex justify-content-center gap-2">
                  <Badge bg="outline-primary" text="primary" className="border">K10: {qd.SoLopK10}</Badge>
                  <Badge bg="outline-success" text="success" className="border">K11: {qd.SoLopK11}</Badge>
                  <Badge bg="outline-warning" text="warning" className="border">K12: {qd.SoLopK12}</Badge>
                </div>
              </td>
              <td className="text-center">
                <div className="btn-group" role="group">
                  <Button variant="outline-primary" size="sm" onClick={() => onEdit(qd)} title="Sửa quy định" className="me-1"><FaEdit /></Button>
                  <Button variant="outline-danger" size="sm" onClick={() => onDelete(qd)} title="Xóa quy định"><FaTrashAlt /></Button>
                </div>
              </td>
            </tr>
          ))}
...
const QuyDinhManagement = () => {
  ...
  const fetchData = useCallback(async (searchQuery) => {
    ...
    try {
      ...
      toast.error("Không thể tải dữ liệu quy định.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async (formData) => {
    try {
      if (modalMode === "create") {
        ...
        toast.success("Thêm niên khóa và quy định mới thành công!");
      } else {
  ...
  return (
    <Container fluid className="py-4">
      ...
      <Row className="justify-content-between align-items-center mb-4">
        <Col xs={12} md="auto">
          <h2 className="h4 mb-2 mb-md-0"><FaClipboardList className="me-2 text-primary" />Các quy định theo niên khóa</h2>
        </Col>
        <Col xs={12} md={5} lg={4}>
          <InputGroup>
            <InputGroup.Text><FaSearch /></InputGroup.Text>
            <Form.Control type="search" placeholder="Tìm theo tên niên khóa..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </InputGroup>
        </Col>
        <Col xs="auto" className="mt-2 mt-md-0">
          <Button variant="primary" onClick={handleShowCreateModal}><FaPlus className="me-2" />Thêm mới</Button>
        </Col>
      </Row>

      {/* Alerts đã được xóa khỏi đây */}
        ...
        <Card className="shadow-sm">
          <Card.Header className="bg-white py-3 border-bottom">
            <div className="d-flex justify-content-between align-items-center">
              <h5 className="mb-0"><FaClipboardList className="me-2 text-primary" />Danh sách quy định ({quyDinhs.length})</h5>
        ...
        <Card className="text-center py-5">
          <Card.Body>
            ...
            <h5 className="text-muted">
              {searchTerm ? `Không tìm thấy quy định cho niên khóa "${searchTerm}"` : "Chưa có quy định nào được thiết lập"}
            </h5>
            {!searchTerm && (
              <Button variant="primary" onClick={handleShowCreateModal} className="mt-3"><FaPlus className="me-2" />Thêm quy định đầu tiên</Button>
            )}
          </Card.Body>
        </Card>
      )}
